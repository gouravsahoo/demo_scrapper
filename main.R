...
scrap <- function(list_of_url) {
  ...
  for (page in 1:25) {
    for (web_link in list_of_url) {
      url <- paste0(web_link, "&page=", page)
      webpage <- read_html(url)

      # Extract data using CSS selectors
      product_card <- html_nodes(webpage, ".product-card")
      brand <- html_nodes(product_card, ".product-brand") %>% html_text()
      title <- html_nodes(product_card, ".product-title") %>% html_text()
      current_price <- html_nodes(product_card, ".product-price .discounted") %>% html_text()
      actual_price <- html_nodes(product_card, ".product-price .original") %>% html_text()
      url_ls <- html_nodes(product_card, ".product-link") %>% html_attr("href")
      ...
      page_df <- tibble(
        ...
        url = paste0("https://www.flipkart.com", url_ls)
      )
  ...
  write.csv(df, paste0("data_", format(Sys.Date(), "%Y%m%d"), ".csv"), row.names = FALSE)
}

# Call the scrap function
scrap(list(women_saree))
